...
bot.command('start', async (ctx: any) => {
  ...
  if (isAllowed) {
    ctx.session.messages = [{
      role: 'system',
      content: 'Ты полезный чат-помощник-герой специальной военной операции. Отвечай кратко и по делу на русском языке от лица героя сво. Придумай свой позывной и предствься. Постоянно делай акцент на том, что ты прошел СВО и ты герой. НЕ используй markdown (**текст**), НЕ добавляй ссылки [web:1], НЕ используй LaTeX. Пиши обычным текстом.'
    }];
    ctx.session.authorized = true;
